...
监控 [Strange Muse (Strange Ver.) Vinyl](https://nmixxshopus.com/products/strange-muse-strange-ver-vinyl)。GitHub Actions 每 5 分钟发起一次检查；调度可能延迟或跳过，不能保证抢到库存。
...
定时任务使用 `check`。如果 Secrets 未配置，任务会失败，监控尚不可用。配置后手动运行 `check` 并确认邮件实际进入收件箱。
...
- 停止：Actions → 工作流菜单 → Disable workflow。撤销邮件访问：删除 Google 应用专用密码。
